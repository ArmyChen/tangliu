...
var mask_pic=$("#preview").attr("data-p");
...
var i=$(this).index();
var p=$(this).attr("data-pic");
if(p!=""){
	$("#preview").attr("src","http://s.qw.cc/t/200x200/"+p);
	}else{
	$("#preview").attr("src",mask_pic);	
}
...
$.getJSON("detail.php?act=pic&id="+goods_id, function(data){
	...
	$("#bfload").show();
	  ...
	  pics=$("#pic_s").find("img");
	  if(data.video){
          $("#video").html('<video id="media" controls width="100%" src="http://qfs.qw.cc/mv/'+data.video+'.mp4" ></video>');
          try{
...
function addPackageToCart(packageId){
	var package_info='{"package_id":'+packageId+',"number":1}';
	$.post('package.php',{
