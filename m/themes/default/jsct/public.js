...
var page = (function(win) {
	var $html = $('html'), $wrapper = $("#wrapper"), $body = $('body'), scale = 1;
	var init = function() {
	...
	$(win).on("orientationchange resize", function(e) {
		init();
	}).resize();
	return {
		init: init,
...
//输入关键词，失去焦点和获取焦点
function oInput(obj){
	$(obj).children('input').focus(function(){
		if(!this.initPlaceholder){
			this.initPlaceholder = this.placeholder;
		}
		if(this.placeholder === this.initPlaceholder){
			this.placeholder = '';
		}
	}).blur(function(){
...
current('.listPage .rankNav');
current('.allList .leftNav');
current('.goods .gift a');
current('.goods .affiliate a');

//点击轮换消失事件
function cardDisply(obj){
	$(obj).find('.cardDisply>li:gt(0)').hide();
	$(obj).find('.opened').click(function(){
		$(this).parent().parent().find('.cardDisply>li:gt(0)').show();	
	});
	$(obj).find('.closed').click(function(){
		$(this).parent().parent().find('.cardDisply>li:gt(0)').hide();	
	});
}
cardDisply('.gift');
cardDisply('.affiliate');


//选项卡事件
function card(obj){
	$(obj).find('.card>li').hide();
	$(obj).find('.card>li:eq(0)').show();
	$(obj).find('.nav>li').click(function(){
		$(obj).find('.card>li').hide();
		var i = $(this).index();
		$(obj).find('.card>li:eq('+i+')').show();
	})	
};
card('.allList');

/*提交按钮效果*/
$('.clickSubmit').mousedown(function(){
	$(this).css('height','100%');	
});
$('.clickSubmit').mouseup(function(){
	$(this).css('height','90%');	
});
