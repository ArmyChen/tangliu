...
( '$fromUsername', '$keyword', $time);");
                ...
                if(empty($wxch_user_wxid)){
                    if(empty($wxch_user_name)){
                        ...
                        if(empty($wxch_nobd_wxid)){
                            $wxch_user_sql = "INSERT INTO `$thistable` ( `user_name`,`password`,`wxid`,`user_rank`,`site_bd`) VALUES ('$fromUsername','$ec_pwd','$fromUsername','99','no')";
            ...
            if($postObj -> Event == 'subscribe'){
                if(empty($postObj -> EventKey)){
                    ...
                    $scan_ret = $db -> getRow("SELECT function FROM `site_qr` WHERE scene_id =$scene_id");
...
( '$fromUsername', '$menu_message', $time);");
            ...
            if($keyword == 'bd'){
                ...
                if($setp == 3){
                    ...
                    $user_id = $db -> getOne("SELECT `user_id` FROM `$table2` WHERE `site_bd` = 'ok' AND `wxid` ='$fromUsername'");
                    $orders = $db -> getAll("SELECT * FROM `$thistable` WHERE `user_id` = '$user_id' ORDER BY `order_id` DESC LIMIT 0,5");
                }else{
                    $thistable = $db -> prefix . 'order_info';
                    $table2 = $db -> prefix . 'users';
                    $user_id = $db -> getOne("SELECT `user_id` FROM `$table2` WHERE `site_bd` = 'no' AND `wxid` ='$fromUsername'");
                    $orders = $db -> getAll("SELECT * FROM `$thistable` WHERE `user_id` = '$user_id' ORDER BY `order_id` DESC LIMIT 0,5");
                }
...
nation=大雁塔&mode=driving&region=西安';
    ...
    protected function plusPoint($db, $uname, $keyword, $fromUsername){
        ...
        $num = $db -> getOne("SELECT `point_num` FROM `site_point` WHERE `point_name` = 'qiandao'");
        $lasttime = time();
        if(empty($record)){
            $dateline = time();
