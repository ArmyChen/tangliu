        ...
        $.getJSON('index.php?act=cart&op=del&cart_id=' + cart_id, function(result){
            if(result.state){
                ...
                if(result.quantity == 0){//判断购物车是否为空
                	...
                	$('div[nc_group="'+cart_id+'"]').remove();//移除本商品或本套装
            		if (parent_tr.children('div').length == 2) {//只剩下店铺名头和店铺合计尾，则全部移除
            		    parent_tr.remove();
            		}
            		calc_cart_price();
                }
...
function change_quantity(cart_id, input){
    ...
    var subtotals = $('#input_item_' + cart_id );

    //暂存为局部变量，否则如果用户输入过快有可能造成前后值不一致的问题
    var _value = input.value;

	if(isNaN(_value) || _value<1){		
		$('#input_item_'+cart_id).val('1');
		_value=1;
	}
    ...
    $.getJSON('index.php?act=cart&op=update&cart_id=' + cart_id + '&quantity=' + _value, function(result){
    	...
    	if(result.state == 'true'){
            ...
            subtotals.val(result.goods_num);
        ...
        if(result.state == 'invalid'){
          ...
          showDialog(result.msg, 'error','','','1','','','','','',2);
          return;
        }
...
function decrease_quantity(cart_id){
    ...
    var common_id = $('#item' + cart_id+'_goods_common_id').val();
    tracecart(common_id,"-1");
...
function calc_cart_price() {
    //每个店铺商品价格小计
    obj = $('div');
    if(obj.children('.shop_list').length==0) return;
    //购物车已选择商品的总价格
    var allTotal = 0;
	var sunm = 0;
	var goods_sum = 0;	
    obj.children('.shop_list').each(function(){
        //购物车每个店铺已选择商品的总价格
        var eachTotal = 0;	
		...
		$(this).find('input[type="checkbox"]').each(function(){		
			...
			eachTotal = eachTotal + parseFloat($(this).parent().parent().find('em[nc_type="eachGoodsTotal"]').html());  
			goods_sum = goods_sum + parseFloat($(this).parent().parent().find('em[nc_type="goods_price"]').html())*$(this).parent().parent().find('input[name="number"]').val();  
			sunm=sunm+1;			
        });
        allTotal += eachTotal;
        $(this).children('tr').last().find('em[nc_type="eachStoreTotal"]').eq(0).html(number_format(eachTotal,2));
    });
	var youhui=goods_sum-allTotal;
	
    $('#cartTotal').html(number_format(allTotal,2));
	$('#goods_sum').html(number_format(goods_sum,2));
	$('#youhui').html(number_format(youhui,2));
	$('#sunm').html(sunm);
}
$(function(){
    calc_cart_price();
