...
{
    $smarty->assign('ur_here',     $_LANG['ad_position_pc']);
    $smarty->assign('action_link', array('text' => $_LANG['mad_position_pc_add'], 'href' => 'ad_position_pc.php?act=add'));
    ...
    assign_query_info();
    $smarty->display('ad_position_list_pc.htm');
}
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    /* 模板赋值 */
    $smarty->assign('ur_here',     $_LANG['mad_position_pc_add']);
    $smarty->assign('form_act',    'insert');

    $smarty->assign('action_link', array('href' => 'ad_position_pc.php?act=list', 'text' => $_LANG['ad_position_pc']));
    $smarty->assign('posit_arr',   array('position_style' => '<table cellpadding="0" cellspacing="0">' ."\n". '{foreach from=$ads item=ad}' ."\n". '<tr><td>{$ad}</td></tr>' ."\n". '{/foreach}' ."\n". '</table>'));

    assign_query_info();
    $smarty->display('ad_position_info_pc.htm');
}
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if ($exc->num("position_name", $position_name) == 0)
    {
        ...
        $link[1]['href'] = 'ad_position_pc.php?act=add';

        $link[2]['text'] = $_LANG['back_position_list'];
        $link[2]['href'] = 'ad_position_pc.php?act=list';
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ($db->getOne($sql) == 0)
    {
        ...
        if ($db->query($sql))
        {
           ...
           /* 提示信息 */
           $link[] = array('text' => $_LANG['back_position_list'], 'href' => 'ad_position_pc.php?act=list');
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('ad_position_list_pc.htm'), '',
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $url = 'ad_position_pc.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
