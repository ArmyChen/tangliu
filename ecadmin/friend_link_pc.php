...
{
    /* 模板赋值 */
    $smarty->assign('ur_here',     $_LANG['flink_list_pc']);
    $smarty->assign('action_link', array('text' => $_LANG['mflink_list_pc_add'], 'href' => 'friend_link_pc.php?act=add'));
...
{
    admin_priv('flink_list_pc');

    $smarty->assign('ur_here',     $_LANG['mflink_list_pc_add']);
    $smarty->assign('action_link', array('href'=>'friend_link_pc.php?act=list', 'text' => $_LANG['flink_list_pc']));
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if ($exc->num("link_name", $link_name) == 0)
    {
        ...
        /* 记录管理员操作 */
        admin_log($_POST['link_name'], 'add', 'flink_list_pc');
        ...
        $link[0]['href'] = 'friend_link_pc.php?act=add';

        $link[1]['text'] = $_LANG['back_list'];
        $link[1]['href'] = 'friend_link_pc.php?act=list';
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $smarty->assign('action_link', array('href'=>'friend_link_pc.php?act=list&' . list_link_postfix(), 'text' => $_LANG['flink_list_pc']));
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $db->query($sql);
    /* 记录管理员操作 */
    admin_log($_POST['link_name'], 'edit', 'flink_list_pc');
    ...
    $link[0]['href'] = 'friend_link_pc.php?act=list&' . list_link_postfix();
...
{
    check_authz_json('flink_list_pc');
    ...
    else
    {
        ...
        {
            admin_log($link_name, 'edit', 'flink_list_pc');
            clear_cache_files();
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $exc->drop($id);
    clear_cache_files();
    admin_log('', 'remove', 'flink_list_pc');

    $url = 'friend_link_pc.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
