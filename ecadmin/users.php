...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    if ($row)
    {
		...
		$user['useraddressinfo']      = get_user_address($row['user_id']);
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT user_id,wxid, user_name, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time ".
    ...
    for ($i=0; $i<$count; $i++)
    {
       ...
       $user_list[$i]['userinfo'] = get_user_wxinfo( $user_list[$i]['wxid']);
...
/* 获得文章的信息 */
    $sql = "SELECT * ".
            "FROM " .$GLOBALS['ecs']->table('weixin_user'). " AS a ".
            "WHERE  uid = ".$userid." ";
    $row = $GLOBALS['db']->getRow($sql);
