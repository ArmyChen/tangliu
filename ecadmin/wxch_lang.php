...
if($_REQUEST['act'] == 'add') 
{
	if($_POST)
	{
		...
		if($type == 1)
		{
			...
			$db->query("INSERT INTO `site_lang` (`name`, `keyword`, `type`, `contents`, `count`, `status`) VALUES
('$name', '$keyword', $type, '$contents', 0, 1);");
		...
		elseif($type == 2)
		{
			...
			$db->query("INSERT INTO `site_lang` (`name`, `keyword`, `type`, `p_title`, `path`, `summary`, `contents`, `count`, `status`) VALUES
...
elseif($_REQUEST['act'] == 'edit') 
{
	...
	if($_POST) 
	{
		...
		$sql = "UPDATE  `site_lang` SET  `lang_value` =  '$lang_value' WHERE  `lang_id` = $id";
	...
	{
		$id = $_GET['id'];
		$data = $db->getRow("SELECT * FROM `site_lang` WHERE `lang_id` = $id");
...
elseif($_REQUEST['act'] == 'remove') 
{
	...
	$filter['start'] = $start;
	$ret = $db->getAll("SELECT * FROM `site_lang` LIMIT $start , $filter[page_size]");
	$wxchdata = array();
	foreach($ret as $k=>$v)
	{
		if($v['type'] == 1)
...
elseif($_REQUEST['act'] == 'query') 
{
	if(!empty($_POST['keyword']))
	{
		...
		$filter['start'] = $start;
		$ret = $db->getAll("SELECT * FROM `site_lang` WHERE `name` LIKE '%$keyword%' LIMIT $start , $filter[page_size]");
		$wxchdata = array();
		foreach($ret as $k=>$v)
		{
			if($v['type'] == 1)
