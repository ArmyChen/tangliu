...
if ($_REQUEST['act'] == 'list') 
{
	...
	$record_count = $db->getOne("SELECT count( uid ) FROM `site_weixin_user` ");
	$filter['page'] = 1;
	$filter['page_size'] = 15;
	$full_page = 1;
	if($filter['page'] <=1)
	...
	$sql = $sql = "SELECT * FROM `site_weixin_user` ORDER BY `dateline` DESC LIMIT $start , $filter[page_size]";
	$ret = $db->getAll($sql);
	foreach($ret as $k=>$v)
	{
		...
		$v['ec_user_name'] = $db->getOne("SELECT `user_name` FROM ".$ecs->table('users')." WHERE `site_bd` = 'no' AND `wxid` = '$v[wxid]'");
		$wdata[] = $v;
		switch ($v['sex']) 
		{
...
elseif($_REQUEST['act'] == 'send') 
{
	...
	if(!empty($uid)) 
	{
		$sql = "SELECT * FROM `site_weixin_user` WHERE `uid` = '$uid'";
		$w_user = $db->getRow($sql);
		$sql = "SELECT * FROM `site_message` WHERE `wxid` = '$w_user[wxid]' ORDER BY `dateline` DESC LIMIT 0,5";
		$ret = $db->getAll($sql);
	}
	elseif(!empty($wxid)) 
	{
		$sql = "SELECT * FROM `site_weixin_user` WHERE `wxid` = '$wxid'";
		$w_user = $db->getRow($sql);
		$sql = "SELECT * FROM `site_message` WHERE `wxid` = '$wxid' ORDER BY `dateline` DESC LIMIT 0,5";
		$ret = $db->getAll($sql);
	}
	$message = array();
	if(count($ret)) 
...
elseif($_REQUEST['act'] == 'view') 
{
	...
	if(!empty($uid)) 
	{
		$sql = "SELECT * FROM `site_weixin_user` WHERE `uid` = '$uid'";
		$w_user = $db->getRow($sql);
		$record_count = $db->getOne("SELECT count( id ) FROM `site_message` WHERE `wxid` = '$w_user[wxid]' ");
		$filter['page'] = 1;
		$filter['page_size'] = 15;
		$full_page = 1;
		if($filter['page'] <=1)
		...
		$filter['start'] = $start;
		$filter['type'] = $_REQUEST['act'];
		$ret = $db->getAll("SELECT * FROM `site_message` WHERE `wxid` = '$w_user[wxid]' ORDER BY `dateline` DESC LIMIT $start , $filter[page_size] ");
		$wxchdata = array();
		foreach($ret as $k=>$v)
		{
...
elseif ($_REQUEST['act'] == 'remove') 
{
	...
	if($type == 'view') 
	{
		...
		$del_sql = "DELETE FROM `site_message` WHERE `id` = '$id';";
		$db->query($del_sql);
		...
		$id = intval($_REQUEST['id']);
		$del_sql = "DELETE FROM `site_weixin_user` WHERE `uid` = '$id';";
		$db->query($del_sql);
		$del_sql2 = "DELETE FROM `site_users` WHERE `user_id` = '$id';";
		$db->query($del_sql2);
...
elseif($_REQUEST['act'] == 'query') 
{
	...
	if(!empty($_POST['keyword']))
	{
		...
		$filter['record_count'] = $db->getOne("SELECT count( wxid ) FROM `site_message` WHERE `message` LIKE '%$keyword%' ");
		...
		$filter['type'] = 'view';
		$ret = $db->getAll("SELECT * FROM `site_message` WHERE `message` LIKE '%$keyword%' ORDER BY `dateline` DESC LIMIT $filter[start] , $filter[page_size]");
		$wxchdata = array();
		foreach($ret as $k=>$v)
		{
			$v['dateline'] = date("H:i",$v['dateline']);
	...
	elseif(!empty($_POST['keywords'])) 
	{
		...
		$filter['record_count'] = $db->getOne("SELECT count( wxid ) FROM `site_weixin_user` WHERE `$row_type` LIKE '%$keyword%' ");
		if (isset($_REQUEST['page_size']) && intval($_REQUEST['page_size']) > 0) 
		{
			$filter['page_size'] = intval($_REQUEST['page_size']);
		}
		...
		$filter['start'] = ($filter['page'] - 1) * $filter['page_size'];
		$filter['type'] = 'view';
		$sql = "SELECT * FROM `site_weixin_user` WHERE `$row_type` LIKE '%$keyword%' ORDER BY `dateline` DESC LIMIT $filter[start] , $filter[page_size]";
		$ret = $db->getAll($sql);
		foreach($ret as $k=>$v)
		{
			$v['subscribe_time'] = date("Y-m-d",$v['subscribe_time']);
	...
	elseif($type == 'list') 
	{
		...
		$filter['start'] = ($filter['page'] - 1) * $filter['page_size'];
		$filter['type'] = 'list';
		$sql = "SELECT * FROM `site_weixin_user` ORDER BY `dateline` DESC LIMIT $filter[start] , $filter[page_size]";
		$ret = $db->getAll($sql);
		foreach($ret as $k=>$v)
		{
			$v['subscribe_time'] = date("Y-m-d",$v['subscribe_time']);
	...
	elseif($type == 'view') 
	{
		...
		$filter['start'] = ($filter['page'] - 1) * $filter['page_size'];
		$filter['type'] = 'view';
		$ret = $db->getAll("SELECT * FROM `site_message` WHERE `wxid` = '$w_user[wxid]' ORDER BY `dateline` DESC LIMIT $filter[start] , $filter[page_size]");
		$wxchdata = array();
		foreach($ret as $k=>$v)
		{
			$v['dateline'] = date("H:i",$v['dateline']);
