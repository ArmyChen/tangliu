...
{
 admin_priv('weixin_wxconfig');
	$smarty->assign('ur_here',     '微信接口');
	if(!empty($_POST['token']))
	{
		$token = $_POST['token'];
		$appid = $_POST['appid'];
		$appsecret = $_POST['appsecret'];
		$ret = $db->query("UPDATE `site_weixin_config` SET `token`='$token',`appid`='$appid',`appsecret`='$appsecret' WHERE `id`=1;");
...
elseif($_REQUEST['act'] == 'autoreg') 
{
 admin_priv('weixin_wxconfig');
	...
	if($_POST) 
	{
		...
		$cfg_sql = "UPDATE site_weixin_cfg SET `cfg_value` = '$userpwd' WHERE `cfg_name` = 'userpwd';";
		$update_sql = "UPDATE site_weixin_autoreg SET `autoreg_name` = '$auto_reg',`autoreg_rand` = '$rand',`autoreg_content`='$content',`state`=$state WHERE `autoreg_id` =1;";
		$db->query($cfg_sql);
		$ret = $db->query($update_sql);
		$link[] = array('href' =>'wxch-ent.php?act=autoreg', 'text' => '自动注册');
		if($ret)
		{
			sys_msg('设置成功',0,$link);
		}
	...
	{
		$cfg = $db->getRow("SELECT * FROM site_weixin_cfg WHERE `cfg_name` = 'userpwd'");
		$data = $db->getRow("SELECT * FROM site_weixin_autoreg WHERE `autoreg_id` = 1");
	}
...
elseif($_REQUEST['act'] == 'menu') 
{
	...
	$smarty->assign('ur_here',     '微信菜单设置');
	if($_POST) 
	{
		$level = 1;
		$aid = 0;
		$db->query('TRUNCATE TABLE `site_weixin_menu`');
		$first_type = $_POST['first_type'];
		$first = $_POST['first'];
		$first_value = $_POST['first_value'];
		foreach($first as $k=>$v)
		{
			$sql = "INSERT INTO `site_weixin_menu` (`menu_type`, `level`, `name`, `value`, `aid`) VALUES ( '$first_type[$k]', $level, '$first[$k]', '$first_value[$k]', $aid)";
			$db->query($sql);
		}
		$level = 2;
		$aid = 1;
		...
		{
			$sql = "INSERT INTO `site_weixin_menu` (`menu_type`, `level`, `name`, `value`, `aid`) VALUES ( '$menu_type1[$k]', $level, '$second1[$k]', '$value1[$k]', $aid)";
			$db->query($sql);
		}
		$aid = 2;
		...
		{
			$sql = "INSERT INTO `site_weixin_menu` (`menu_type`, `level`, `name`, `value`, `aid`) VALUES ( '$menu_type2[$k]', $level, '$second2[$k]', '$value2[$k]', $aid)";
			$db->query($sql);
		}
		$aid = 3;
		...
		foreach($second2 as $k=>$v)
		{
			$sql = "INSERT INTO `site_weixin_menu` (`menu_type`, `level`, `name`, `value`, `aid`) VALUES ( '$menu_type3[$k]', $level, '$second3[$k]', '$value3[$k]', $aid)";
			$db->query($sql);
		}
	...
	{
		$data = array();
		$sql = "SELECT * FROM  `site_weixin_menu` WHERE  `aid` =0";
		$data['first'] = $db->getAll($sql);
		$sql = "SELECT * FROM  `site_weixin_menu` WHERE  `aid` =1";
		...
		$sql = "SELECT * FROM  `site_weixin_menu` WHERE  `aid` =2";
		...
		$sql = "SELECT * FROM  `site_weixin_menu` WHERE  `aid` =3";
...
elseif($_REQUEST['act'] == 'config') 
{
 admin_priv('weixin_config');
	$smarty->assign('ur_here',     $_LANG['mweixin_config']);
	if($_POST)
	{
		$data = array();
		...
		foreach($data as $k=>$v) 
		{
			$sql = "UPDATE  site_weixin_cfg SET  `cfg_value` =  '$v' WHERE  `cfg_name` = '$k';";
			$db->query($sql);
		}
	...
	{
		$ret = $db->getAll("SELECT * FROM  site_weixin_cfg ");
		$wxchdata = array();
		foreach($ret as $k=>$v)
		{
			$wxchdata[$k] = $v;
...
{	
	 admin_priv('weixin_keywords_gz');
	if($_POST)
	{	
		$id=$_POST['id'];
		if($id==3){
			$update_sql = "UPDATE  `site_weixin_keywords1` SET  `is_start` = 0 WHERE  `type` =4;";
			$db->query($update_sql);				
		}else{
		
			$update_sql = "UPDATE  `site_weixin_keywords1` SET  `is_start` = 0 WHERE  `type` =3;";
			$db->query($update_sql);
		}
		$update_sql = "UPDATE  `site_weixin_keywords1` SET  `is_start` = 1 WHERE  `type` ='$id';";
		$db->query($update_sql);
		$link[] = array('href' =>'wxch-ent.php?act=regmsg', 'text' => '消息自动回复');
		sys_msg('切换成功',0,$link);
	}else{
	$smarty->assign('ur_here',     '关注自动回复');
	$del_sql = "SELECT * FROM `site_weixin_keywordswx` ORDER BY `site_weixin_keywordswx`.`id` DESC LIMIT 0,19";
	$ret = $db->getAll($del_sql);
	
	$sql1 = "SELECT type FROM `site_weixin_keywords1` where is_start=1";
	$ret1 = $db->getOne($sql1);
	...
	foreach($ret as $k)
	{
		...
		{
			$db->query("DELETE FROM `site_weixin_keywordswx` WHERE `site_weixin_keywordswx`.`id` = $k[id]");
		}
	}
	$record_count = $db->getOne("SELECT count( id ) FROM `site_weixin_keywordswx` ");
	$full_page = 1;
	...
	$filter['start'] = $start;
	$filter['type'] = 'list';
	$ret = $db->getAll("SELECT * FROM `site_weixin_keywordswx` LIMIT $start , $filter[page_size]");
	$wxchdata = array();
	foreach($ret as $k=>$v)
	{
		if($v['type'] == 1)
...
elseif($_REQUEST['act'] == 'lang') 
{
	...
	$ret = $db->getAll("SELECT * FROM `site_weixin_lang`");
	$record_count = $db->getOne("SELECT count( lang_id ) FROM `site_weixin_lang` ");
	$filter['page'] = 1;
	$filter['page_size'] = 30;
	$full_page = 1;
	if($filter['page'] <=1)
	...
	$filter['page_count'] = $page_count;
	$filter['start'] = $start;
	$sql = "SELECT * FROM `site_weixin_lang` LIMIT $start , $filter[page_size]";
	$ret = $db->getAll($sql);
	$wxchdata = array();
	foreach($ret as $k=>$v)
	{
...
{
	 admin_priv('weixin_keywords');
	$smarty->assign('ur_here',     $_LANG['mweixin_keywords']);
	$del_sql = "SELECT * FROM `site_weixin_keywordswx` ORDER BY `site_weixin_keywordswx`.`id` DESC LIMIT 0,19";
	$ret = $db->getAll($del_sql);
	foreach($ret as $k)
	{
...
{
		 admin_priv('weixin_oauth');
			$smarty->assign('ur_here',     $_LANG['mweixin_oauth']);
	$record_count = $db->getOne("SELECT count( oid ) FROM site_weixin_oauth ");
	$full_page = 1;
	if (isset($_REQUEST['page_size']) && intval($_REQUEST['page_size']) > 0) 
	{
		$filter['page_size'] = intval($_REQUEST['page_size']);
	...
	$ret = $db->getAll("SELECT * FROM site_weixin_oauth LIMIT $filter[start] , $filter[page_size]");
	$wxchdata = array();
	foreach($ret as $k=>$v)
	{
...
elseif($_REQUEST['act'] == 'point') 
{
	...
	if($_POST) 
	{
		...
		foreach($point_name as $k=>$v)
		{
			...
			$sql = "UPDATE `site_weixin_point` SET  `point_value` = $point_value[$v],`point_num` = '$point_num[$v]',`autoload` =  '$autoload[$v]' WHERE  `point_name` ='$point_name[$k]';";
			$db->query($sql);
		}
	...
	{
		$sql = "SELECT * FROM `site_weixin_point`";
		$ret = $db->getAll($sql);
		foreach ($ret as $k => $v) 
		{
			switch ($v['point_name']) 
...
elseif($_REQUEST['act'] == 'prize') 
{
	...
	$record_count = $db->getOne("SELECT count( pid ) FROM `site_weixin_prize` ");
	$filter['page'] = 1;
	$filter['page_size'] = 15;
	$full_page = 1;
	if ($filter['page'] <= 1) 
	...
	$filter['start'] = $start;
	$filter['type'] = 'prize';
	$ret = $db->getAll("SELECT * FROM `site_weixin_prize` ORDER BY `dateline` DESC LIMIT $start , $filter[page_size]");
	$wxchdata = array();
	foreach ($ret as $k => $v) 
	{
...
elseif($_REQUEST['act'] == 'zjd') 
{
	...
	$record_count = $db->getOne("SELECT count( wxid ) FROM `site_weixin_prize_users` WHERE `fun` = 'egg' AND `yn` = 'yes' ");
	$full_page = 1;
	if (isset($_REQUEST['page_size']) && intval($_REQUEST['page_size']) > 0) 
	{
		$filter['page_size'] = intval($_REQUEST['page_size']);
	...
	$filter['page_count'] = $page_count;
	$filter['type'] = 'zjd';
	$ret = $db->getAll("SELECT * FROM `site_weixin_prize_users` WHERE `fun` = 'egg' AND `yn` = 'yes' ORDER BY `dateline` ASC LIMIT $start , $filter[page_size]");
	$wxchdata = array();
	foreach($ret as $k=>$v)
	{
...
elseif($_REQUEST['act'] == 'dzp') 
{
	...
	$record_count = $db->getOne("SELECT count( wxid ) FROM `site_weixin_prize_users` WHERE `fun` = 'dzp' AND `yn` = 'yes' ");
	$full_page = 1;
	if (isset($_REQUEST['page_size']) && intval($_REQUEST['page_size']) > 0) 
	{
		$filter['page_size'] = intval($_REQUEST['page_size']);
	...
	$filter['page_count'] = $page_count;
	$filter['type'] = 'dzp';
	$ret = $db->getAll("SELECT * FROM `site_weixin_prize_users` WHERE `fun` = 'dzp' AND `yn` = 'yes' ORDER BY `dateline` ASC LIMIT $start , $filter[page_size]");
	$wxchdata = array();
	foreach($ret as $k=>$v)
	{
		$v['dateline'] = date("Y-m-d H:i:s",$v['dateline']);
...
elseif($_REQUEST['act'] == 'ggk') 
{
	 admin_priv('weixin_oauth');
	$wxch_lang['ur_here'] = '大转盘';
	$record_count = $db->getOne("SELECT count( wxid ) FROM `site_weixin_prize_users` WHERE `fun` = 'ggk' AND `yn` = 'yes' ");
	$full_page = 1;
	if (isset($_REQUEST['page_size']) && intval($_REQUEST['page_size']) > 0) 
	{
		$filter['page_size'] = intval($_REQUEST['page_size']);
	...
	$filter['start'] = $start;
	$filter['type'] = 'dzp';
	$ret = $db->getAll("SELECT * FROM `site_weixin_prize_users` WHERE `fun` = 'ggk' AND `yn` = 'yes' ORDER BY `dateline` ASC LIMIT $start , $filter[page_size]");
	$wxchdata = array();
	foreach($ret as $k=>$v)
	{
		$v['dateline'] = date("Y-m-d H:i:s",$v['dateline']);
...
elseif($_REQUEST['act'] == 'fun') 
{
	...
	$ret = $db->getAll("SELECT * FROM site_weixin_msg");
	$record_count = $db->getOne("SELECT count( id ) FROM site_weixin_msg ");
	$filter['page'] = 1;
	$filter['page_size'] = 15;
	$full_page = 1;
	if($filter['page'] <=1)
	...
	$filter['page_count'] = $page_count;
	$filter['start'] = $start;
	$sql = "SELECT * FROM site_weixin_msg LIMIT $start , $filter[page_size]";
	$ret = $db->getAll($sql);
	$wxchdata = array();
	foreach($ret as $k=>$v)
	{
...
elseif($_REQUEST['act'] == 'qr') 
{
	...
	$_SESSION['act_uri'] = 'qr';
	$record_count = $db->getOne("SELECT count( qid ) FROM site_weixin_qr WHERE `type` = 'qr'");
	$full_page = 1;
	if (isset($_REQUEST['page_size']) && intval($_REQUEST['page_size']) > 0) 
	{
		$filter['page_size'] = intval($_REQUEST['page_size']);
	...
	$filter['start'] = $start;
	$filter['type'] = 'qr';
	$ret = $db->getAll("SELECT * FROM site_weixin_qr WHERE `type` = 'qr' ORDER BY `qid` DESC LIMIT $filter[start] , $filter[page_size]");
	$wxchdata = array();
	foreach($ret as $k=>$v)
	{
...
elseif($_REQUEST['act'] == 'order') 
{
	...
	if($_POST) 
	{
		...
		if($image) 
		{
			$sql = "UPDATE `site_weixin_order` SET `title` = '$title',`image` = '$image',`autoload` = '$autoload' WHERE `id` = 1;";
		}
		elseif($http_ret1 or $http_ret2) 
		{
			$sql = "UPDATE `site_weixin_order` SET `title` = '$title',`image` = '$img_url',`autoload` = '$autoload' WHERE `id` = 1;";
		}
		else 
		{
			$sql = "UPDATE `site_weixin_order` SET `title` = '$title',`autoload` = '$autoload' WHERE `id` = 1;";
		}
		$db->query($sql);
	...
	{
		$sql = "SELECT * FROM `site_weixin_order` WHERE `id` = 1";
		$ret =$db->getRow($sql);
...
elseif($_REQUEST['act'] == 'pay') 
{
	...
	if($_POST) 
	{
		...
		if($image) 
		{
			$sql = "UPDATE `site_weixin_order` SET `title` = '$title',`image` = '$image',`autoload` = '$autoload' WHERE `id` = 3;";
		}
		elseif($http_ret1 or $http_ret2) 
		{
			$sql = "UPDATE `site_weixin_order` SET `title` = '$title',`image` = '$img_url',`autoload` = '$autoload' WHERE `id` = 3;";
		}
		else 
		{
			$sql = "UPDATE `site_weixin_order` SET `title` = '$title',`autoload` = '$autoload' WHERE `id` = 3;";
		}
		$db->query($sql);
		$href = 'wxch-ent.php?act='.$act;
	...
	{
		$sql = "SELECT * FROM `site_weixin_order` WHERE `id` = 3";
		$ret =$db->getRow($sql);
...
elseif($_REQUEST['act'] == 'reorder') 
{
	...
	if($_POST) 
	{
		...
		if($image) 
		{
			$sql = "UPDATE `site_weixin_order` SET `title` = '$title',`image` = '$image',`autoload` = '$autoload' WHERE `id` = 2;";
		}
		elseif($http_ret1 or $http_ret2) 
		{
			$sql = "UPDATE `site_weixin_order` SET `title` = '$title',`image` = '$img_url',`autoload` = '$autoload' WHERE `id` = 2;";
		}
		else 
		{
			$sql = "UPDATE `site_weixin_order` SET `title` = '$title',`autoload` = '$autoload' WHERE `id` = 2;";
		}
		$db->query($sql);
		$href = 'wxch-ent.php?act='.$act;
		$link[] = array('href' => $href, 'text' => $wxch_lang['ur_here']);
	...
	{
		$sql = "SELECT * FROM `site_weixin_order` WHERE `id` = 2";
		$ret =$db->getRow($sql);
		$smarty->assign('wxch_lang',$wxch_lang);
		$smarty->assign('order',$ret);
...
elseif($_REQUEST['act'] == 'guanli') 
{
	...
	$record_count = $db->getOne("SELECT count( aid ) FROM `site_admin`");
	$full_page = 1;
	if (isset($_REQUEST['page_size']) && intval($_REQUEST['page_size']) > 0) 
	{
		$filter['page_size'] = intval($_REQUEST['page_size']);
	...
	$filter['start'] = $start;
	$filter['type'] = 'guanli';
	$ret = $db->getAll("SELECT * FROM `site_admin` ORDER BY `aid` DESC LIMIT $start , $filter[page_size]");
	$wxchdata = array();
	foreach($ret as $k=>$v)
	{
		$v['dateline'] = date("Y-m-d H:i:s",$v['dateline']);
		...
		$v['nickname'] = $db->getOne("SELECT `nickname` FROM `site_weixin_user` WHERE `wxid` = '$v[wxid]'");
		$wxchdata[$k] = $v;
	}
	$filter['record_count'] = $record_count;
	$smarty->assign('wxch_ver',1);
...
elseif($_REQUEST['act'] == 'tuijian') 
{
	...
	$record_count = $db->getOne("SELECT count( qid ) FROM site_weixin_qr WHERE `affiliate` !=0 ");
	$filter['page'] = 1;
	$filter['page_size'] = 15;
	$full_page = 1;
	if($filter['page'] <=1)
	...
	$filter['start'] = $start;
	$filter['type'] = 'qr';
	$ret = $db->getAll("SELECT * FROM site_weixin_qr  WHERE `affiliate` !=0 ORDER BY `qid` DESC LIMIT $start , $filter[page_size]");
	$wxchdata = array();
	foreach($ret as $k=>$v)
	{
		if($v['action_name'] == 'QR_LIMIT_SCENE')
...
elseif($_REQUEST['act'] == 'muli_user') 
{
	...
	$record_count = $db->getOne("SELECT count( * ) FROM `site_muli_user` ");
	$filter['page'] = 1;
	$filter['page_size'] = 15;
	$full_page = 1;
	if($filter['page'] <=1)
	...
	$filter['start'] = $start;
	$filter['type'] = 'qr';
	$ret = $db->getAll("SELECT * FROM `site_muli_user`  ORDER BY `id` DESC LIMIT $start , $filter[page_size]");
	$filter['record_count'] = $record_count;
...
elseif($_REQUEST['act'] == 'messages') 
{
	if(!empty($_POST['regmsg']))
	{
		...
		$sql = "SELECT * FROM `site_weixin_lang` WHERE `lang_name` = 'regmsg'";
		$ret =$db->getOne($sql);
		if($ret)
		{
			$sql = "UPDATE `site_weixin_lang` SET `lang_value` = '$lang_value' WHERE `lang_name` = '$lang_name'";
			$ret = $db->query($sql);
		...
		{
			$sql = "INSERT INTO `site_weixin_lang` (`lang_name` ,`lang_value`) VALUES ( '$lang_name', '$lang_value')";
			$ret = $db->query($sql);
			$link[] = array('href' =>'wxch-ent.php?act=regmsg', 'text' => '关注回复设置');
			if($ret)
			{
	...
	else
	{
		...
		$sql = "SELECT `lang_value` FROM `site_weixin_lang` WHERE `lang_name` = 'regmsg'";
		$ret =$db->getOne($sql);
...
elseif ($_REQUEST['act'] == 'toggle_show') 
{
	...
	$db->query("UPDATE `site_weixin_keywordswx` SET `status` = '$val' WHERE `id` =$id;");
	clear_cache_files();
...
elseif ($_REQUEST['act'] == 'prize_show') 
{
	$id = intval($_POST['id']);
	$val = intval($_POST['val']);
	$db->query("UPDATE `site_weixin_prize` SET `status` = '$val' WHERE `id` =$id;");
	clear_cache_files();
	make_json_result($val);
}
elseif ($_REQUEST['act'] == 'zjd_show') 
{
	$id = intval($_POST['id']);
	$val = intval($_POST['val']);
	$db->query("UPDATE `site_weixin_prize_users` SET `status` = '$val' WHERE `id` =$id;");
	clear_cache_files();
	make_json_result($val);
}
elseif ($_REQUEST['act'] == 'zjd_state') 
{
	$pid = intval($_POST['id']);
	$val = intval($_POST['val']);
	$db->query("UPDATE `site_weixin_prize` SET `status` = '$val' WHERE `pid` =$pid;");
	clear_cache_files();
	make_json_result($val);
}
elseif ($_REQUEST['act'] == 'toggle_guanli') 
{
	$id = intval($_POST['id']);
	$val = intval($_POST['val']);
	$db->query("UPDATE `site_admin` SET `autoload` = '$val' WHERE `aid` =$id;");
	clear_cache_files();
	make_json_result($val);
}
elseif ($_REQUEST['act'] == 'remove') 
{
	...
	if($type == 'list') 
	{
		...
		$del_sql = "DELETE FROM `site_weixin_keywordswx` WHERE `id` = '$kws_id';";
		$db->query($del_sql);
	...
	elseif($type == 'zjd') 
	{
		...
		$del_sql = "DELETE FROM `site_weixin_prize_users` WHERE `fun` = 'egg' AND `id` = '$id';";
		$db->query($del_sql);
	...
	{
		$id = intval($_REQUEST['id']);
		$del_sql = "DELETE FROM `site_weixin_prize_users` WHERE `fun` = 'dzp' AND `id` = '$id';";
		$db->query($del_sql);
		$url = 'wxch-ent.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
		ecs_header("Location: $url\n");
		exit;
	...
	{
		$id = intval($_REQUEST['id']);
		$del_sql = "DELETE FROM `site_weixin_prize` WHERE `pid` = '$id';";
		$db->query($del_sql);
		$url = 'wxch-ent.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
		ecs_header("Location: $url\n");
		exit;
	...
	elseif($type == 'qr') 
	{
		...
		$del_sql = "DELETE FROM site_weixin_qr WHERE `qid` = '$qid';";
		$db->query($del_sql);
		$url = 'wxch-ent.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
		ecs_header("Location: $url\n");
		exit;
	...
	{
		$id = intval($_REQUEST['id']);
		$del_sql = "DELETE FROM site_weixin_oauth WHERE `oid` = '$id';";
		$db->query($del_sql);
		$url = 'wxch-ent.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
		ecs_header("Location: $url\n");
		exit;
	...
	{
		$id = intval($_REQUEST['id']);
		$del_sql = "DELETE FROM `site_admin` WHERE `aid` = '$id';";
		$db->query($del_sql);
		$url = 'wxch-ent.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
		ecs_header("Location: $url\n");
		exit;
...
elseif ($_REQUEST['act'] == 'query') 
{
	...
	if($goto == 'search') 
	{
		...
		{
			$filter['type'] = 'guanli';
			$ret = $db->getAll("SELECT * FROM `site_admin` WHERE `name` LIKE '%$keyword%' ORDER BY `aid` DESC LIMIT $filter[start] , $filter[page_size]");
			$wxchdata = array();
			foreach($ret as $k=>$v)
			{
				$v['dateline'] = date("Y-m-d H:i:s",$v['dateline']);
	...
	if(!empty($keyword)) 
	{
		if($type == 'oauth') 
		{
			$record_count = $db->getOne("SELECT count( oid ) FROM site_weixin_oauth WHERE `name` LIKE '%$keyword%' ");
			$page_count = ceil($record_count / $filter['page_size']);
			$filter['page_count'] = $page_count;
			$filter['record_count'] = $record_count;
			if(empty($keyword))
			...
			{
				$ret = $db->getAll("SELECT * FROM site_weixin_oauth WHERE `name` LIKE '%$keyword%' LIMIT $filter[start] , $filter[page_size]");
			}
			$cfg_baseurl = $db->getOne("SELECT cfg_value FROM site_weixin_cfg WHERE cfg_name = 'baseurl'");
			$oauth_url = $cfg_baseurl.'wechat/oauth/wxch_oauth.php?oid=';
			$wxchdata = array();
		...
		elseif($type=='prize') 
		{
			$record_count = $db->getOne("SELECT count( pid ) FROM `site_weixin_prize` WHERE `title` LIKE '%$keyword%' ");
			$page_count = ceil($record_count / $filter['page_size']);
			$filter['page_count'] = $page_count;
			$filter['record_count'] = $record_count;
			if(empty($keyword))
			{
				$search_sql = "SELECT * FROM `site_weixin_prize` ORDER BY `dateline` DESC LIMIT $filter[start] , $filter[page_size]";
			}
			else
			{
				$search_sql = "SELECT * FROM `site_weixin_prize` WHERE `title` LIKE '%$keyword%' ORDER BY `dateline` DESC LIMIT $filter[start] , $filter[page_size]";
			}
		...
		elseif($type == 'qr') 
		{
			...
			if($act_uri == 'qr') 
			{
				$filter['record_count'] = $db->getOne("SELECT count( qid ) FROM site_weixin_qr WHERE `type` = 'qr'");
				...
				{
					$ret = $db->getAll("SELECT * FROM site_weixin_qr WHERE `type` = 'qr' AND `scene` LIKE '%$keyword%' AND `type` = 'qr' ORDER BY `qid` DESC LIMIT $filter[start] , $filter[page_size]");
				}
				$wxchdata = array();
				foreach($ret as $k=>$v)
				{
			...
			{
				$filter['record_count'] = $db->getOne("SELECT count( qid ) FROM site_weixin_qr WHERE `type` = 'tj'");
				$filter['page_count'] = ceil($filter['record_count'] / $filter['page_size']);
				if(empty($keyword))
				{
					$ret = $db->getAll("SELECT * FROM site_weixin_qr WHERE `type` = 'tj' ORDER BY `qid` DESC LIMIT $filter[start] , $filter[page_size] ");
				}
				else
				{
					$ret = $db->getAll("SELECT * FROM site_weixin_qr WHERE `type` = 'tj' AND `scene` LIKE '%$keyword%' ORDER BY `qid` DESC LIMIT $filter[start] , $filter[page_size]");
				}
				$wxchdata = array();
				foreach($ret as $k=>$v)
				{
		...
		{
			$record_count = $db->getOne("SELECT count( id ) FROM `site_weixin_keywordswx` WHERE `name` LIKE '%$keyword%' ");
			$page_count = ceil($record_count / $filter['page_size']);
			$filter['page_count'] = $page_count;
			$filter['record_count'] = $record_count;
			if(empty($keyword))
			{
				$ret = $db->getAll("SELECT * FROM `site_weixin_keywordswx` LIMIT $filter[start] , $filter[page_size]");
			}
			else
			{
				$ret = $db->getAll("SELECT * FROM `site_weixin_keywordswx` WHERE `name` LIKE '%$keyword%' LIMIT $filter[start] , $filter[page_size]");
			}
			$wxchdata = array();
			foreach($ret as $k=>$v)
			{
	...
	elseif($type == 'list') 
	{
		$filter['start'] = ($filter['page'] - 1) * $filter['page_size'];
		$filter['type'] = 'list';
		$filter['record_count'] = $db->getOne("SELECT count( id ) FROM `site_weixin_keywordswx` WHERE `type` = 'qr'");
		$filter['page_count'] = ceil($filter['record_count'] / $filter['page_size']);
		$ret = $db->getAll("SELECT * FROM `site_weixin_keywordswx` LIMIT $filter[start] , $filter[page_size]");
		$wxchdata = array();
		foreach($ret as $k=>$v)
	...
	elseif($type == 'qr') 
	{
		...
		if($act_uri == 'qr') 
		{
			...
			{
				$ret = $db->getAll("SELECT * FROM site_weixin_qr WHERE `scene` LIKE '%$keyword%' AND `type` = 'qr' ORDER BY `qid` DESC LIMIT $filter[start] , $filter[page_size]");
			}
			$wxchdata = array();
			foreach($ret as $k=>$v)
			{
		...
		elseif($act_uri == 'tuijian') 
		{
			...
			{
				$ret = $db->getAll("SELECT * FROM site_weixin_qr WHERE WHERE `type` = 'tj' `scene` LIKE '%$keyword%' ORDER BY `qid` DESC LIMIT $filter[start] , $filter[page_size]");
			}
			$wxchdata = array();
			foreach($ret as $k=>$v)
			{
	...
	elseif($type == 'prize') 
	{
		$filter['start'] = ($filter['page'] - 1) * $filter['page_size'];
		$filter['type'] = 'prize';
		$filter['record_count'] = $db->getOne("SELECT count( pid ) FROM `site_weixin_prize`");
		$filter['page_count'] = ceil($filter['record_count'] / $filter['page_size']);
		$keyword = $_POST['keyword'];
		if(empty($keyword))
		{
			$ret = $db->getAll("SELECT * FROM `site_weixin_prize` LIMIT $filter[start] , $filter[page_size]");
		}
		else
		{
			$ret = $db->getAll("SELECT * FROM `site_weixin_prize` WHERE `title` LIKE '%$keyword%' LIMIT $filter[start] , $filter[page_size]");
		}
		$wxchdata = array();
		foreach($ret as $k=>$v)
		{
	...
	elseif($type == 'oauth') 
	{
		...
		$filter['page_count'] = ceil($filter['record_count'] / $filter['page_size']);
		$filter['type'] = 'oauth';
		$filter['record_count'] = $db->getOne("SELECT count( oid ) FROM site_weixin_oauth");
		$ret = $db->getAll("SELECT * FROM site_weixin_oauth LIMIT $filter[start] , $filter[page_size]");
		$cfg_baseurl = $db->getOne("SELECT cfg_value FROM site_weixin_cfg WHERE cfg_name = 'baseurl'");
		$oauth_url = $cfg_baseurl.'wechat/oauth/wxch_oauth.php?oid=';
		$wxchdata = array();
	...
	elseif($type == 'guanli') 
	{
		$filter['start'] = ($filter['page'] - 1) * $filter['page_size'];
		$filter['type'] = 'guanli';
		$filter['record_count'] = $db->getOne("SELECT count( aid ) FROM `site_admin`");
		$filter['page_count'] = ceil($filter['record_count'] / $filter['page_size']);
		$ret = $db->getAll("SELECT * FROM `site_admin` ORDER BY `aid` DESC LIMIT $filter[start] , $filter[page_size]");
		$wxchdata = array();
		foreach($ret as $k=>$v)
		{
			$v['dateline'] = date("Y-m-d H:i:s",$v['dateline']);
	...
	elseif($type == 'zjd') 
	{
		$filter['start'] = ($filter['page'] - 1) * $filter['page_size'];
		$filter['type'] = 'zjd';
		$filter['record_count'] = $db->getOne("SELECT count( wxid ) FROM `site_weixin_prize_users` WHERE `fun` = 'egg' AND `yn` = 'yes' ");
		$filter['page_count'] = ceil($filter['record_count'] / $filter['page_size']);
		if(empty($keyword))
		{
			$ret = $db->getAll("SELECT * FROM `site_weixin_prize_users` WHERE `fun` = 'egg' AND `yn` = 'yes' ORDER BY `dateline` ASC LIMIT $filter[start] , $filter[page_size]");
		}
		else
		{
			$ret = $db->getAll("SELECT * FROM `site_weixin_prize_users` WHERE `fun` = 'egg' AND `prize_name` AND `yn` = 'yes' LIKE '%$keyword%' ORDER BY `dateline` ASC LIMIT $filter[start] , $filter[page_size]");
		}
		$wxchdata = array();
		foreach($ret as $k=>$v)
		{
	...
	elseif($type == 'dzp') 
	{
		$filter['start'] = ($filter['page'] - 1) * $filter['page_size'];
		$filter['type'] = 'dzp';
		$filter['record_count'] = $db->getOne("SELECT count( wxid ) FROM `site_weixin_prize_users` WHERE `fun` = 'dzp' AND `yn` = 'yes' ");
		$filter['page_count'] = ceil($filter['record_count'] / $filter['page_size']);
		if(empty($keyword))
		{
			$ret = $db->getAll("SELECT * FROM `site_weixin_prize_users` WHERE `fun` = 'dzp' AND `yn` = 'yes' ORDER BY `dateline` ASC LIMIT $filter[start] , $filter[page_size]");
		}
		else
		{
			$ret = $db->getAll("SELECT * FROM `site_weixin_prize_users` WHERE `fun` = 'dzp' AND `prize_name` AND `yn` = 'yes' LIKE '%$keyword%' ORDER BY `dateline` ASC LIMIT $filter[start] , $filter[page_size]");
		}
		$wxchdata = array();
		foreach($ret as $k=>$v)
		{
