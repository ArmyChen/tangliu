...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->display('payment_list_pc.htm');
    ...
    $smarty->assign('action_link', array(
        ...
        'href' => 'payment_pc.php?act=list'
    ));
    $smarty->assign('pay', $pay);
    $smarty->display('payment_edit_pc.htm');
    ...
    if (is_string($pay['pay_config'])) {
        ...
        $pay['pay_config'] = array();
        foreach ($data['config'] as $key => $value) {
            $pay['pay_config'][$key]['desc'] = (isset($_LANG[$value['name'] . '_desc'])) ?  'alipay_desc' : '';
    ...
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('payment') . " WHERE pay_name = '$_POST[pay_name]' AND pay_code <> '$_POST[pay_code]'";
    if ($db->GetOne($sql) > 0) {
    ...
    if ($_POST['pay_id']) {
        $sql = "UPDATE " . $ecs->table('payment') . "SET pay_name = '$_POST[pay_name]'," . "    pay_desc = '$_POST[pay_desc]'," . "    pay_config = '$pay_config', " . "    pay_fee    =  '$pay_fee' " . "WHERE pay_code = '$_POST[pay_code]' LIMIT 1";
        $db->query($sql);
        ...
        if ($db->GetOne($sql) > 0) {
            $sql = "UPDATE " . $ecs->table('payment') . "SET pay_name = '$_POST[pay_name]'," . "    pay_desc = '$_POST[pay_desc]'," . "    pay_config = '$pay_config'," . "    pay_fee    =  '$pay_fee', " . "    enabled = '1' " . "WHERE pay_code = '$_POST[pay_code]' LIMIT 1";
            $db->query($sql);
        } else {
            $sql = "INSERT INTO " . $ecs->table('payment') . " (pay_code, pay_name, pay_desc, pay_config, is_cod, pay_fee, enabled, is_online)" . "VALUES ('$_POST[pay_code]', '$_POST[pay_name]', '$_POST[pay_desc]', '$pay_config', '$_POST[is_cod]', '$pay_fee', 1, '$_POST[is_online]')";
            $db->query($sql);
        }
