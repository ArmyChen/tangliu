...
if($_REQUEST['act'] == 'add') 
{
	...
	if($_POST)
	{
		...
		$db->query("INSERT INTO site_weixin_oauth (`name`, `contents`, `count`, `status`) VALUES
('$name', '$contents', 0, 1);");
...
elseif($_REQUEST['act'] == 'edit') 
{
	...
	if($_POST)
	{
		...
		$update_sql = "UPDATE  site_weixin_oauth SET  `name` =  '$name',`contents` =  '$contents' WHERE  `oid` ='$id';";
		$db->query($update_sql);
		$link[] = array('href' =>'wxch-ent.php?act=oauth', 'text' => 'OAuth管理');
		sys_msg('修改成功',0,$link);
	}
	$id = $_GET['oid'];
	$data = $db->getRow("SELECT * FROM site_weixin_oauth WHERE `oid` = $id");
