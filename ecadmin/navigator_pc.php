...
admin_priv('navigator_pc');
$exc = new exchange($ecs->table("nav") , $db, 'id', 'name');
if ($_REQUEST['act'] == 'list') {
    $smarty->assign('ur_here', $_LANG['mnavigator_pc']);
    $smarty->assign('action_link', array(
        'text' => $_LANG['mnavigator_pc_add'],
        'href' => 'navigator_pc.php?act=add'
    ));
    ...
    assign_query_info();
    $smarty->display('navigator_pc.htm');
    ...
    make_json_result($smarty->fetch('navigator_pc.htm') , '', array(
        'filter' => $navdb['filter'],
    ...
    if (empty($_REQUEST['step'])) {
        ...
        $sysmain = get_sysnav();
        $smarty->assign('action_link', array(
            'text' => $_LANG['mnavigator_pc'],
            'href' => 'navigator_pc.php?act=list'
        ));
        $smarty->assign('ur_here', $_LANG['mnavigator_pc']);
        assign_query_info();
        $smarty->assign('sysmain', $sysmain);
        $smarty->assign('rt', $rt);
        $smarty->display('navigator_add_pc.htm');
    } elseif ($_REQUEST['step'] == 2) {
        $item_name = $_REQUEST['item_name'];
        $item_url = $_REQUEST['item_url'];
		  $item_durl = $_REQUEST['durl'];
        ...
        if ($item_ifshow == 1 && $item_type == 'middle') {
            ...
            if ($arr) {
                ...
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,durl,item_ifxiala,type) VALUES('$item_name','" . $arr['type'] . "','" . $arr['id'] . "','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_durl','$item_ifxiala','$item_type')";
            }
        }
        if (empty($sql)) {
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('nav') . " (name,ifshow,vieworder,opennew,url,durl,type) VALUES('$item_name','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_durl','$item_type')";
        }
        $db->query($sql);
        clear_cache_files();
        $links[] = array(
    ...
    if (empty($_REQUEST['step'])) {
        ...
        $rt['item_vieworder'] = $row['vieworder'];
		 $rt['item_durl'] = $row['durl'];
		     $rt['item_ifxiala_'.$row['ifxiala']] = 'selected';
        ...
        $item_name = $_REQUEST['item_name'];
        $item_url = $_REQUEST['item_url'];
		  $item_durl = $_REQUEST['item_durl'];
		   $item_ifxiala = $_REQUEST['item_ifxiala'];
        $item_ifshow = $_REQUEST['item_ifshow'];
        $item_opennew = $_REQUEST['item_opennew'];
		 $item_ifxiala = $_REQUEST['item_ifxiala'];
        $item_type = $_REQUEST['item_type'];
  ...
  $sql = "UPDATE " . $GLOBALS['ecs']->table('nav') . " SET name='$item_name',ctype='',cid='',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',durl='$item_durl',ifxiala='$item_ifxiala',type='$item_type' WHERE id='$id'";
        }
        $db->query($sql);
        clear_cache_files();
        $links[] = array(
