...
elseif($_REQUEST['act'] == 'add') 
{
	if($_POST)
	{
		...
		{
		$keyword="关注回复文本";
		$sql = 'SELECT id FROM  site_weixin_keywords1 where type=3';
		$id=$db->getOne($sql);
		$type=3;
		$contents = strip_tags($_POST['contents'],"<a>");
		if($id){
			$update_sql = "UPDATE  `site_weixin_keywords1` SET  `name` =  '$name',`keyword` =  '$keyword',`type` =  '$type',`name` =  '$name',`contents` =  '$contents' WHERE  `id` ='$id';";
		$db->query($update_sql);
		}else{
			$contents = htmltowei($contents);
			$db->query("INSERT INTO `site_weixin_keywords1` (`name`, `keyword`, `type`, `contents`, `count`, `status`) VALUES
		...
		elseif($type == 2)
		{
			$sql = 'SELECT id FROM  site_weixin_keywords1 where type=4';
			$id=$db->getOne($sql);
			if(empty($id)){
			...
			if(!empty($kws_id))
			{
				$db->query("UPDATE  `site_weixin_keywords1` SET  `name` =  '$name',`keyword` = '$keyword',`type` = '$type',`status` =  '1' WHERE `id` =$kws_id;");
			}
		}else{
			$name ="关注回复图文";
			$keyword = "关注回复图文";
	...
	if(empty($id)){
		...
		if($_GET['type'] == 'text')
		{
			$site_lang['ur_here'] = '添加文字信息';
			$smarty->display('wxch_keywords1_infotext.html');
		}
		elseif($_GET['type'] == 'image')
		{
			$site_lang['ur_here'] = '添加图文信息';
			$smarty->display('wxch_keywords1_infoimage.html');
		}	
	}else{

			$data = $db->getRow("SELECT * FROM `site_weixin_keywords1` WHERE `id` = $id");
			if($_GET['type'] == 'image') 
			{
...
elseif($_REQUEST['act'] == 'edit') 
{
	if($_POST)
	{
		...
		$type = $_POST['type'];
		$contents = strip_tags($_POST['contents'],"<a>");
		$update_sql = "UPDATE  `site_keywords` SET  `name` =  '$name',`keyword` =  '$keyword',`type` =  '$type',`name` =  '$name',`contents` =  '$contents' WHERE  `id` ='$id';";
		$db->query($update_sql);
	...
	$id = $_GET['id'];
	$data = $db->getRow("SELECT * FROM `site_keywords` WHERE `id` = $id");
	if($_GET['type'] == 'image') 
	{
		$article_list = get_keywords_articles($id, $db);
	}
...
elseif($_REQUEST['act'] == 'remove') 
{
	...
	$filter['start'] = $start;
	$ret = $db->getAll("SELECT * FROM `site_keywords` LIMIT $start , $filter[page_size]");
	$wxchdata = array();
	foreach($ret as $k=>$v)
	{
		if($v['type'] == 1)
...
elseif ($_REQUEST['act'] == 'add_article') 
{
	...
	{
		$insert_sql = "INSERT INTO `site_keywords` (`name`) VALUES
('');";
		$db->query($insert_sql);
		$kws_id = $db->insert_id();
	...
	{
		$sql = "INSERT INTO site_keywords_article (kws_id, article_id) VALUES ('$kws_id', '$val')";
		$db->query($sql);
	}
...
elseif ($_REQUEST['act'] == 'drop_article') 
{
	...
	foreach ($articles AS $val) 
	{
		$sql = "DELETE FROM `site_keywords_article` WHERE `site_keywords_article`.`article_id` = $val;";
		$db->query($sql);
	}
...
elseif($_REQUEST['act'] == 'query') 
{
	if(!empty($_POST['keyword']))
	{
		...
		$filter['start'] = $start;
		$ret = $db->getAll("SELECT * FROM `site_keywords` WHERE `name` LIKE '%$keyword%' LIMIT $start , $filter[page_size]");
		$wxchdata = array();
		foreach($ret as $k=>$v)
		{
			if($v['type'] == 1)
