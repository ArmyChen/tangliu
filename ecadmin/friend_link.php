...
$exc = new exchange($ecs->table('touch_friend_link'), $db, 'link_id', 'link_name');
...
{
    /* 模板赋值 */
    $smarty->assign('ur_here',     $_LANG['flink_list_mobile']);
    $smarty->assign('action_link', array('text' => $_LANG['mflink_list_mobile_add'], 'href' => 'friend_link.php?act=add'));
...
{
    admin_priv('flink_list_mobile');

    $smarty->assign('ur_here',     $_LANG['mflink_list_mobile_add']);
    $smarty->assign('action_link', array('href'=>'friend_link.php?act=list', 'text' => $_LANG['flink_list_mobile']));
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if ($exc->num("link_name", $link_name) == 0)
    {
        ...
        /* 插入数据 */
        $sql    = "INSERT INTO ".$ecs->table('touch_friend_link')." (link_name, link_url, link_logo, show_order) ".
                  "VALUES ('$link_name', '$link_url', '$link_logo', '$show_order')";
        $db->query($sql);

        /* 记录管理员操作 */
        admin_log($_POST['link_name'], 'add', 'flink_list_mobile');
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT link_id, link_name, link_url, link_logo, show_order ".
           "FROM " .$ecs->table('touch_friend_link'). " WHERE link_id = '".intval($_REQUEST['id'])."'";
    ...
    $smarty->assign('action_link', array('href'=>'friend_link.php?act=list&' . list_link_postfix(), 'text' => $_LANG['flink_list_mobile']));
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    {
        //获取链子LOGO,并删除
        $old_logo = $db->getOne("SELECT link_logo FROM " .$ecs->table('touch_friend_link'). " WHERE link_id = '$id'");
    ...
    /* 更新信息 */
    $sql = "UPDATE " .$ecs->table('touch_friend_link'). " SET ".
            "link_name = '$link_name', ".
            "link_url = '$link_url' ".
            $link_logo.','.
    ...
    $db->query($sql);
    /* 记录管理员操作 */
    admin_log($_POST['link_name'], 'edit', 'flink_list_mobile');
...
{
    check_authz_json('flink_list_mobile');
    ...
    else
    {
        ...
        {
            admin_log($link_name, 'edit', 'flink_list_mobile');
            clear_cache_files();
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $exc->drop($id);
    clear_cache_files();
    admin_log('', 'remove', 'flink_list_mobile');
...
function get_links_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 获得总记录数据 */
        $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('touch_friend_link');
        ...
        /* 获取数据 */
        $sql  = 'SELECT link_id, link_name, link_url, link_logo, show_order'.
               ' FROM ' .$GLOBALS['ecs']->table('touch_friend_link').
