...
{
    /* 模板赋值 */
    $smarty->assign('ur_here',     $_LANG['madmin_role']);
    $smarty->assign('action_link', array('href'=>'role.php?act=add', 'text' => $_LANG['madmin_role_add']));
...
{
    /* 检查权限 */
    admin_priv('admin_role_add');
    include_once(ROOT_PATH . 'lang/' .$_CFG['lang']. '/admin/priv_action.php');

    $priv_str = '';

    /* 获取权限的分组数据 */
    $sql_query = "SELECT action_id, parent_id, action_code, relevance,is_nshow FROM " .$ecs->table('admin_action').
                 " WHERE parent_id = 0 and is_nshow = 0";
    ...
    foreach ($priv_arr AS $action_id => $action_group)
    {
     ...
     /* 模板赋值 */
    $smarty->assign('ur_here',     $_LANG['madmin_role_add']);
    $smarty->assign('action_link', array('href'=>'role.php?act=list', 'text' => $_LANG['madmin_role']));
...
{
    check_authz_json('admin_role_remove');
