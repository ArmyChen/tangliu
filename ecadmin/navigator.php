...
admin_priv('navigator_mobile');
$exc = new exchange($ecs->table("touch_nav") , $db, 'id', 'name');
if ($_REQUEST['act'] == 'list') {
    $smarty->assign('ur_here', $_LANG['mnavigator_mobile']);
    $smarty->assign('action_link', array(
        'text' => $_LANG['mnavigator_mobile_add'],
    ...
    if (empty($_REQUEST['step'])) {
        ...
        if ($item_ifshow == 1 && $item_type == 'middle') {
            ...
            if ($arr) {
                ...
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('touch_nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,durl,type) VALUES('$item_name','" . $arr['type'] . "','" . $arr['id'] . "','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_durl','$item_type')";
            }
        }
        if (empty($sql)) {
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('touch_nav') . " (name,ifshow,vieworder,opennew,url,durl,type) VALUES('$item_name','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_durl','$item_type')";
        }
        $db->query($sql);
        clear_cache_files();
        $links[] = array(
    ...
    if (empty($_REQUEST['step'])) {
        ...
        if ($arr) {
            ...
            $sql = "UPDATE " . $GLOBALS['ecs']->table('touch_nav') . " SET name='$item_name',ctype='',cid='',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',durl='$item_durl',pic='$item_pic',type='$item_type' WHERE id='$id'";
        }
        $db->query($sql);
        clear_cache_files();
        $links[] = array(
