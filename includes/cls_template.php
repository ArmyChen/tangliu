...
$cid = $GLOBALS['db']->getOne("select cat_id from ". $GLOBALS['ecs']->table("category") ." where define_url='".$cid."' limit 0,1"); 
...
//通过参数判断是否存在二级分类获取文章列表$cat_arr[$idx]['children'] = get_clild_article_content_listok($row['cat_id']);





function get_article_urlok($url)
...
$cid = $GLOBALS['db']->getOne("select cat_id from ". $GLOBALS['ecs']->table("article_cat") ." where define_url='".$cid."'  limit 0,1"); 
...
$sql = "select * from ".$GLOBALS['ecs']->table('article_cat')." where cat_id=".$cid."      ORDER BY sort_order ASC, cat_id ASC";
...
$res = $GLOBALS['db']->getAll($sql);
...

  {
   ...
   $cat_arr[$idx]['children'] = get_clild_article_listok($row['cat_id']);
...
function get_clild_article_listok($pid)

{

   //开始获取子分类

    $sql_sub = "select * from ".$GLOBALS['ecs']->table('article')." where cat_id=".$pid."  and is_open=1  order by add_time desc ";
...

 {
...

  {
   ...
   $children[$sidx]['id']=$subrow['article_id'];

   $children[$sidx]['name']=$subrow['title'];

   $children[$sidx]['url']=build_uri('article', array('aid' => $subrow['article_id']), $subrow['title']);
...
function get_article_urlok_com($url)
...
$res = $GLOBALS['db']->getAll($sql);
...

  {
   ...
   $cat_arr[$idx]['children'] = get_clild_article_listok_com($row['cat_id']);
...
function get_clild_article_listok_com($pid)

{

   //开始获取子分类

    $sql_sub = "select * from ".$GLOBALS['ecs']->table('article')." where cat_id=".$pid."  and is_open=1 and article_type_key=1 order by add_time desc ";
...

 {
...

  {
   ...
   $children[$sidx]['file_url']=$subrow['file_url'];

   $children[$sidx]['spcdesc']=$subrow['spcdesc'];
...
function get_article_cat_urlok($url)
...
$sql = "select * from ".$GLOBALS['ecs']->table('article_cat')." where parent_id=".$cid."     ORDER BY sort_order ASC, cat_id ASC";
...
$cat_arr[$idx]['url']  = build_uri('article_cat', array('acid' => $row['cat_id']), $row['cat_name']);
...
function get_category_cat_urlok($url)
{
 $rs = strstr($url, "cat-");
 if($rs!==false)
 {
...
$sql = "select * from ".$GLOBALS['ecs']->table('category')." where parent_id=".$cid."     ORDER BY sort_order ASC, cat_id ASC";
...
function get_clild_article_cat_listok($pid)

{

   //开始获取子分类

    $sql_sub = "select * from ".$GLOBALS['ecs']->table('article_cat')." where parent_id=".$pid."     ORDER BY sort_order ASC, cat_id ASC ";
...

 {
...

  {
   ...
   $children[$sidx]['url']=build_uri('article_cat', array('acid' => $subrow['cat_id']), $subrow['cat_name']);
...
function get_article_cat_urlok_com($url)
...
$sql = "select * from ".$GLOBALS['ecs']->table('article_cat')." where parent_id=".$cid."  and article_type_key=1   ORDER BY sort_order ASC, cat_id ASC";
...
$cat_arr[$idx]['file_url'] = $row['file_url'];
...

{
    ...
    $sql = "SELECT a.*".
            ...
            "WHERE  a.ad_id = '$ad_id' GROUP BY a.ad_id";
