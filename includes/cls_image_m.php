...
define('ROOT_PATH_M',ROOT_PATH.'m/');
class cls_image
{
    var $error_no    = 0;
    var $error_msg   = '';
    ...
    function upload_image($upload, $dir = '', $img_name = '')
    {
        ...
        if (empty($dir))
        {
            /* 创建当月目录 */
            $dir = date('Ym');
            $dir = ROOT_PATH_M . $this->images_dir . '/' . $dir . '/';
        }
        else
        {
            /* 创建目录 */
            $dir = ROOT_PATH_M . $this->data_dir . '/' . $dir . '/';
            if ($img_name)
            {
        ...
        {
            return str_replace(ROOT_PATH_M, '', $img_name);
        }
        else
        {
    ...
    function make_thumb($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor='')
    {
        ...
        {
            return str_replace(ROOT_PATH_M, '', str_replace('\\', '/', realpath($img)));
        }
        ...
        if (empty($path))
        {
            $dir = ROOT_PATH_M . $this->images_dir . '/' . date('Ym').'/';
        }
        else
        {
            $dir = $path;
        ...
        {
            return str_replace(ROOT_PATH_M, '', $dir) . $filename;
        }
        else
        {
    ...
    function add_watermark($filename, $target_file='', $watermark='', $watermark_place='', $watermark_alpha = 0.65)
    {
        ...
        {
            return str_replace(ROOT_PATH_M, '', str_replace('\\', '/', realpath($filename)));
        }
        ...
        if ($path)
        {
            return str_replace(ROOT_PATH_M, '', str_replace('\\', '/', $path));
        }
        else
        {
            $this->error_msg = $GLOBALS['_LANG']['writting_failure'];
