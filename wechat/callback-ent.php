...
class wechatCallbackapi {
	...
	public function responseMsg($db, $user, $base_url) {
		...
		if (!empty($postStr) or $debug == 1) {
			...
			$q_name = $db -> getOne("SELECT `autoreg_name` FROM `site_autoreg` WHERE `autoreg_id` = 1");
			if(empty($q_name)){
				$q_name="weixin";
			}
			$affiliate_id = $db -> getOne("SELECT `affiliate` FROM `site_user` WHERE `wxid` = '$fromUsername'");
			if ($affiliate_id >= 1) {
			...
			$subscribe = 1;
			if ($msgType == 'text') {
				$db -> query("INSERT INTO `site_message` (`wxid`, `message`, `dateline`) VALUES
( '$fromUsername', '$keyword', $time);");
			} 
			$belong = $db -> insert_id();
			$thistable = $db -> prefix . 'users';
			$ec_pwd = $db -> getOne("SELECT `cfg_value` FROM `site_cfg` WHERE `cfg_name` = 'userpwd'");
			$autoreg_rand = $db -> getOne("SELECT `autoreg_rand` FROM `site_autoreg` WHERE `autoreg_id` = 1");
			...
			if (strlen($ret_22['user_name']) == 28) {
				$sql_del = "UPDATE `$thistable` SET `user_rank` = '99',`site_bd`='no' WHERE `wxid` ='$fromUsername'";
				$db -> query($sql_del);
			} 
			$zhanghaoinfo="";
			if (empty($uname)) {
				$wxch_user_name_sql = "SELECT `user_name` FROM `$thistable` WHERE `site_bd`='ok' AND `wxid` = '$fromUsername'";
				...
				if (empty($wxch_user_wxid)) {
					if (empty($wxch_user_name)) {
						$wxch_nobd_wxid_sql = "SELECT `wxid` FROM `$thistable` WHERE `site_bd`='no' AND `wxid` = '$fromUsername'";
						...
						if (empty($wxch_nobd_wxid)) {
							...
							if ($uc_state) {
								...
								$wxch_user_sql = "INSERT INTO `$thistable` (`user_id`,`user_name`,`password`,`wxid`,`user_rank`,`site_bd`) VALUES ('$ecs_user_id','$uc_username','$ecs_password','$fromUsername','99','no')";
								$db -> query($wxch_user_sql);
							} else {
							
								$autoreg_state = $db -> getOne("SELECT `state` FROM `site_autoreg` WHERE `autoreg_id` = 1");
								if($autoreg_state){
									$wxch_user_sql = "INSERT INTO `$thistable` ( `user_name`,`password`,`wxid`,`user_rank`,`site_bd`,`password_tianxin`) VALUES ('$fromUsername','$ec_pwd','$fromUsername','99','no','$ec_pwd_no')";
									$db -> query($wxch_user_sql);
									$ecs_user_id = $db -> insert_id();
									...
									$db -> query($ecs_update);
									//注册后默认绑定
									$db->query("UPDATE `site_user` SET `setp`= 3,`uname`='$ecs_user_name' WHERE `wxid`= '$fromUsername';");
									$db->query("UPDATE  ecs_users SET `site_bd`='ok',`wxid`='$fromUsername' WHERE `user_name`='$ecs_user_name'");
					...
					$wxch_user_sql = " UPDATE `$thistable` SET `user_rank` = '99',`site_bd`='no' WHERE `wxid` ='$fromUsername'";
					$db -> query($wxch_user_sql);
				} 
			}
			...
			if ($postObj -> Event == 'subscribe') {
				if (strlen($postObj -> EventKey) == 0) {
					...
					$db -> query("UPDATE `site_qr` SET `subscribe`=`subscribe` + 1 WHERE `scene_id`= '$scene_id';");
					$scan_ret = $db -> getRow("SELECT * FROM `site_qr` WHERE scene_id =$scene_id");
					if ($scene_id >= 1) {
				...
				$qrscene = $postObj -> EventKey;
				$scene_id = $qrscene;
				$update_sql = "UPDATE `site_qr` SET `scan`=`scan` + 1 WHERE `scene_id`= '$scene_id';";
				$db -> query("$update_sql");				
				$scan_ret = $db -> getRow("SELECT * FROM `site_qr` WHERE scene_id =$scene_id");
				...
				if ($setp > 0 and $setp < 3) {
					...
					if ($setp == 1) {
						...
						if (!$verifyLogin) {
							...
							$affiliate = $db -> getOne("SELECT `affiliate` FROM  `site_user` WHERE `wxid` = '$fromUsername'");
							$users_table = $db -> prefix . 'users';
...
( '$fromUsername', '$menu_message', $time);");
				...
				if (!empty($aff_arr[2])) {
					...
					if(empty($parent_id)){
						if (!empty($aff_db)&&$flag&&$flagetianxin100) {
							...
							/*甜心100新增扫描关注带提醒*/
							$up_uid=$aff_arr[2];
							require(ROOT_PATH . 'site_share.php');
							
							$parent_wxid = $db -> getOne($qu_wxid);
							$qu_name = "SELECT nickname FROM `site_user` WHERE `wxid` = '$parent_wxid'";
			...
			if ($keyword == 'bd') {
				...
				if ($setp == 3) {
					...
					$user_id = $db -> getOne("SELECT `user_id` FROM `$table2` WHERE  `wxid` ='$fromUsername'");
				} else {
					$table2 = $db -> prefix . 'users';
					$user_id = $db -> getOne("SELECT `user_id` FROM `$table2` WHERE  `wxid` ='$fromUsername'");
				} 
				...
				if ($ArticleCount >= 1) {
					...
					foreach($orders as $k => $v) {
						...
						$description = "" . '商品信息：' . $shopinfo . "\r总金额：" . $v['order_amount'] . "" ;
...
$description = '商品信息：' . $shopinfo . "\r\n总金额：" . $orders['order_amount'] . "\r\n" . $pay_status . "\r\n取餐点：" . $orders['address'];
				...
				$table2 = $db -> prefix . 'users';
				if ($setp == 3) {
					$ret = $db -> getRow("SELECT `user_id` FROM `$table2` WHERE `site_bd` = 'ok' AND `wxid` ='$fromUsername'");
					$user_id = $ret['user_id'];
					$orders = $db -> getRow("SELECT * FROM `$thistable` WHERE `user_id` = '$user_id' ORDER BY `order_id` DESC");
				} else {
					$ret = $db -> getRow("SELECT `user_id` FROM `$table2` WHERE `site_bd` = 'no' AND `wxid` ='$fromUsername'");
					$user_id = $ret['user_id'];
					$orders = $db -> getRow("SELECT * FROM `$thistable` WHERE `user_id` = '$user_id' ORDER BY `order_id` DESC");
				} 
				if (empty($orders)) {
				...
				$type = 'tj';
				$qr_path = $db->getOne("SELECT `qr_path` FROM `site_qr` WHERE `scene_id`='$scene_id'");
		...
		if($ticket)
		{
			...
			if(false !==$local_file){
				...
				if(false !==fwrite($local_file,$imageinfo)){
					fclose($local_file);
					//将生成的二维码图片的地址放到数据库中
					$insert_sql = "INSERT INTO `site_qr` (`type`,`action_name`,`ticket`, `scene_id`, `scene` ,`qr_path`,`function`,`affiliate`,`endtime`,`dateline`) VALUES
...
nation=大雁塔&mode=driving&region=西安';
             ...
             <Url><![CDATA[" . $url . "]]></Url>
				...
				$jf_state = $db -> getOne("SELECT `autoload` FROM `site_point` WHERE `point_name` = '$keyword'");
				$msgType = "text";
				if ($jf_state == 'yes') {
					$qd_jf = $db -> getOne("SELECT `point_value` FROM `site_point` WHERE `point_name` = '$keyword'");
				...
				$type1 = $db -> getOne("SELECT `type` FROM `site_keywords1` WHERE `is_start` = 1");
				if($type1==3){
					$keyword="关注回复文本";
				}else{
					$keyword="关注回复图文";
			...
			if (file_exists('site_development.php')) {
				include('site_development.php');
			} 
			if (!empty($keyword)) {
	...
	protected function plusPoint($db, $uname, $keyword, $fromUsername) {
		$res_arr = array();
		$sql = "SELECT * FROM `site_point_record` WHERE `point_name` = '$keyword' AND `wxid` = '$fromUsername'";
		$record = $db -> getRow($sql);
		//甜心100修复积分赠送次数限制
		$num = $db -> getOne("SELECT `point_num` FROM `site_point` WHERE `point_name` = '$keyword'");
		$lasttime = time();
		if (empty($record)) {
			$dateline = time();
			$insert_sql = "INSERT INTO `site_point_record` (`wxid`, `point_name`, `num`, `lasttime`, `datelinie`) VALUES
('$fromUsername', '$keyword' , 1, $lasttime, $dateline);";
			$potin_name = $db -> getOne("SELECT `point_name` FROM `site_point` WHERE `point_name` = '$keyword'");
			if (!empty($potin_name)) {
				$db -> query($insert_sql);
			} 
		} else {
			...
			$time = time();
			$lasttime_sql = "SELECT `lasttime` FROM `site_point_record` WHERE `point_name` = '$keyword' AND `wxid` = '$fromUsername'";
			$db_lasttime = $db -> getOne($lasttime_sql);
			if (($time - $db_lasttime) > (60 * 60 * 24)) {
				$update_sql = "UPDATE `site_point_record` SET `num` = 0,`lasttime` = '$lasttime' WHERE `wxid` ='$fromUsername';";
				$db -> query($update_sql);
			} 
			$record_num = $db -> getOne("SELECT `num` FROM `site_point_record` WHERE `point_name` = '$keyword' AND `wxid` = '$fromUsername'");
			if ($record_num < $num) {
	...
	protected function insert_wmessage($db, $fromUsername, $w_message, $time, $belong) {
		...
		$sql = "INSERT INTO `site_message` (`wxid`, `w_message`, `belong`, `dateline`) VALUES
('$fromUsername', '$w_message', '$belong', '$time');";
	...
	protected function coupon($db, $fromUsername) {
		...
		if (!empty($retc['coupon'])) {
			...
			$ret = $db -> getRow("SELECT * FROM `site_coupon` WHERE `id` = 1");
			$type_id = $ret['type_id'];
	...
	protected function getauto($db, $keyword, $textTpl, $newsTpl, $base_url, $m_url, $fromUsername, $toUsername, $time, $article_url) {
		$this -> universal($fromUsername, $base_url);
		$auto_res = $ret = $db -> getAll("SELECT * FROM `site_keywords`");
		if (count($auto_res) > 0) {
					...
					if ($v['type'] == 1) {
						...
						foreach($res_ks as $kk => $vv) {
							if ($vv == $keyword) {
								...
								$resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
								echo $resultStr;
								$db -> query("UPDATE `site_keywords` SET `count` = `count`+1 WHERE `id` =$v[id]");
								exit;
							} 
						} 
					} elseif ($v['type'] == 2) {
	...
	protected function getauto_reg($db, $keyword, $textTpl, $newsTpl, $base_url, $m_url, $fromUsername, $toUsername, $time, $article_url,$user_name,$ec_pwd_no,$zhanghaoinfo,$user_password) {
		$this -> universal($fromUsername, $base_url);
		$auto_res = $ret = $db -> getAll("SELECT * FROM `site_keywords1`");
		if (count($auto_res) > 0) {
			foreach($auto_res as $k => $v) {
				if ($v['status'] == 1) {
					$res_ks = explode(' ', $v['keyword']);
						...
						foreach($res_ks as $kk => $vv) {
							...
							//增加关注送积分
							 ...
							 if(!empty($g_point)){
								...
								$resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
								echo $resultStr;
								$db -> query("UPDATE `site_keywords1` SET `count` = `count`+1 WHERE `id` =$v[id]");
								exit;
							} 
						} 
					} elseif ($v['type'] == 4) {
