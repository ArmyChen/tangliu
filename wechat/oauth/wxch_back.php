...
$cfg_murl = $db->getOne("SELECT cfg_value FROM site_cfg WHERE cfg_name = 'murl'"); //mobile/
$back_url = $db->getOne("SELECT `contents` FROM `site_oauth` WHERE `oid` = '$oid'");//http://www.mall93.cn/mobile
$affiliate_id = $db->getOne("SELECT `affiliate` FROM `site_user` WHERE `wxid` = '$openid'"); //推荐编号
...
//甜心100修复完善
$update_sql = "UPDATE `site_oauth` SET `count` = `count` + 1 WHERE `oid` = $oid; ";
