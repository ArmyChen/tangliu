...
if(!empty($wxid)) 
{
	...
	if(empty($w_user['nickname'])) 
	{
		...
		{
			$db->query( "UPDATE  `site_user` SET  `nickname` =  'empty' WHERE `wxid` = '$wxid';");
		}
		exit('nickname is empty');
	}
	$ecs_users = $ecs->prefix.'users';
	$w_sql = "UPDATE  `site_user` SET  `nickname` =  '$w_user[nickname]',`sex` =  '$w_user[sex]',`city` =  '$w_user[city]',`country` =  '$w_user[country]',`province` =  '$w_user[province]',`language` =  '$w_user[language]',`headimgurl` =  '$w_user[headimgurl]',`localimgurl` = '$localimgurl', `subscribe_time` =  '$w_user[subscribe_time]' WHERE `wxid` = '$wxid';";
	$db->query($w_sql);
	$user_query = $db->getOne("SELECT `wxid` FROM ".$ecs_users." WHERE `wxid` = `user_name` AND wxid = '$wxid';");
	$w_users = "UPDATE  ".$ecs_users." SET  `user_name` =  '$w_user[nickname]'  WHERE `wxid` = `user_name` AND `site_bd` = 'no' AND `wxid` = '$wxid';";
	...
	else 
	{
		...
		if(empty($users_q)) 
		{
			...
			if(empty($users_q)) 
			{
				$w_users = "UPDATE  ".$ecs_users." SET  `user_name` =  '$w_user1'  WHERE `site_bd` = 'no' AND `wxid` = '$wxid';";
				$db->query($w_users);
			}
		}
	}
